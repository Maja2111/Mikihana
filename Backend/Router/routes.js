...
router.get('/verify/:token', async (req, res) => {
  try {
    ...
    res.json({
      message: 'Email verifield successfully!! You can now log in. ',
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.post('/login', async (req, res) => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    try {
      ...
      if (!isMatch)
        return res.status(400).json({ error: 'Password or Username missed' });
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: 'Fehler beim Senden der E-Mail.' });
    }
...
router.post('/reset-password/:token', async (req, res) => {
  try {
    ...
    if (!newPassword || !newPasswordRepeat) {
      return res
        .status(400)
        .json({ message: 'Bitte beide Passwortfelder ausfüllen.' });
    }
